...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        ...
        eot = tokenized_prompts.argmax(dim=-1).view(n_cls, 1, 1, 1).expand(n_cls, s, 1, dim)
...
class SubspacePromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model, templates, all_classnames):
        super().__init__()
        ...
        # hard_prompt_feature.append(torch.stack(hard_prompt_feature, dim=0).mean(dim=0))
        ...
        self.scale = nn.ParameterList([nn.Parameter(torch.zeros(n_ctx, ctx_dim)) for _ in range(space_dim)])
        # self.bias = nn.ParameterList([nn.Parameter(torch.zeros(ctx_dim)) for _ in range(space_dim)])
        ...
        for i in range(space_dim):
            ...
            with torch.no_grad():
                embedding = clip_model.token_embedding(prompt).type(dtype)
            ctx_vectors = embedding[0, 1 : 1 + n_ctx, :]
            self.bias.append(nn.Parameter(ctx_vectors))

        
        # for single_para in self.bias:
    ...
    def construct_prompts(self, ctx, prefix, suffix, label=None):
        ...
        for i in range(self.space_dim):
            scale = self.scale[i].unsqueeze(0).half()
            bias = self.bias[i].unsqueeze(0).half()
            
            # t = scale * VPT[0]
            # t = t + bias
            t = self.bias[i].unsqueeze(0).expand(self.n_cls, -1, -1)
            VPT.append(t)
        ctx = torch.stack(VPT,dim=1)
        prompts = torch.cat(
            [
    ...
    def forward(self):
        ...
        return prompts # pass here original, as for visual 768 is required
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, templates, all_classnames,):
        ...
        self.prompt_learner = SubspacePromptLearner(cfg, classnames, clip_model, templates, all_classnames)
    ...
    def forward(self, image, label=None):
        ...
        text_features = self.text_encoder(prompts, tokenized_prompts)#(n_cls, space_dim + 1, n_ctx, dim)
        ...
        if self.prompt_learner.training:
            ...
            return logits, F.cross_entropy(logits, label), \
                    ...
                    F.cosine_embedding_loss(hard_prompt_feature.flatten(0,1), projected_hardtext_feature.flatten(0,1), 
                                                torch.ones(hard_prompt_feature.flatten(0,1).size(0)).to(self.device), margin=0.0) 
                    # F.cosine_embedding_loss(hard_feature, hard_projected_feature, 
                    #                        torch.ones(hard_feature.size(0)).to(self.device), margin=0.0)
                   
                #    F.cosine_embedding_loss(zero_projected_image_feature, zero_shot_features, 
                #                            torch.ones(zero_shot_features.size(0)).to(self.device), margin=0.0) 

        else:
            return self.ce_weight * logits + (1 - self.ce_weight) * logits_point
    
     
    def project_space_svd(self, z_query, z_support):
        ...
        z_support = z_support.permute(0,2,1) #C D N

        # u, s, v = torch.svd(z_support.float(), some=True)
        # try:
        u, s, v = torch.linalg.svd(z_support, full_matrices=False)
        # except:
        # with torch.no_grad():
        #     z_mean = z_support.mean()
        # u, s, v = torch.linalg.svd(z_support +  1e-3 * z_mean * torch.randn_like(z_support),full_matrices=False)

        z_support = u

        
        # Work on query vectors
        # z_query [template n_classes n_dim]
        self.beta_hat = torch.matmul(z_support.transpose(1,2), z_query.permute(1,2,0))  # ? C N D * C D B = C N B
        z_lrc = torch.matmul(z_support,self.beta_hat)  # ? C D N * C N B  = C D B
    ...
    def project_space(self, z_query, z_support):
        ...
        if self.use_svd:
            ...
            beta_hat = torch.matmul(z_supports_inv, z_query.permute(1, 2, 0))  # ? [n_classes, n_support, T] ([n_classes, n_support, n_dim] * [n_classes, n_dim, T])
            z_lrc = torch.matmul(z_support.transpose(1, 2), beta_hat)  # n_classes, n_dim, T ([n_classes, n_dim, n_support] * [n_classes, n_support, T])
...
class CustomCLIP_Ens(nn.Module):
    def __init__(self, cfg, classnames, templates, all_classnames, ensemble_num):
        ...
        # n = len(lst)
        # [lst[i * n // k:(i + 1) * n // k] for i in range(k)]
        ...
        for i in range(1,ensemble_num):
            for name, param in ens_clip_model[i].named_parameters():          
                if "VPT" not in name:   
                    ...
                    # 最终通过 set_attr 设置新的参数
                    module_shared = getattr(module_shared, modules[-1])
                    setattr(module, modules[-1], module_shared) 
                                


        self.ensemble_model = nn.ModuleList([CustomCLIP(cfg, classnames, ens_clip_model[i], split_templates[i], all_classnames) 
...
class SuPrEns(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            with autocast():
                output, loss_ce, loss_hard_reg = model(image, label)
                loss = loss_ce + loss_hard_reg * self.cfg.TRAINER.SUPR.REG_LOSS_WEIGHT
            optim.zero_grad()
