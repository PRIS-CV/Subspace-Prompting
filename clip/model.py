...
class ResidualAttentionBlock_TCP(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
        super().__init__()
...
class ResidualAttentionBlock_SuPr(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_prompt=False,
                 text_layer=False, i=0, design_details=None):
        super().__init__()
        ...
        self.text_layer = text_layer
        self.attn_mask = attn_mask
        self.space_dim =  design_details["space_dim"]
        self.max_name_len = design_details["max_name_len"]
        self.pad1 = None
        self.pad2 = None
            ...
            if self.add_prompt:
                if self.text_layer:
                    ...
                    for i in range (self.space_dim):
                        self.register_parameter("VPT_scale{}".format(i), nn.Parameter(torch.zeros(self.n_ctx_text,d_model)))
                        single_para = nn.Parameter(torch.zeros(self.n_ctx_text,d_model))
                        nn.init.normal_(single_para, std=0.02)
                        self.register_parameter("VPT_bias{}".format(i), single_para)
                else:
                    self.n_ctx_visual = design_details["vision_ctx"]  # hyperparameter
                    ctx_vectors = torch.empty(self.n_ctx_visual, d_model)
                # Code snippet for per layer visual prompts
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.add_prompt:
            ...
            if not self.text_layer:
                ...
                VPT = [self.VPT_shallow.half()] #vector prompt
                for i in range(self.space_dim):
                    # for DP not use parameterlist
                    scale = getattr(self, f"VPT_scale{i}").half()
                    bias = getattr(self, f"VPT_bias{i}").half()             
                    ctx = scale * VPT[0]
                    ctx = ctx + bias
                    VPT.append(ctx)
                VPT = torch.stack(VPT) #space_dim + 1, n_ctx, 512
               
                prefix = x[ :, :, :1, :]
                suffix = x[ :, :, 1 + self.n_ctx_text:, :]
                # Create/configure learnable tokens of this layer
                ctx = VPT.expand(x.shape[0], -1, -1, -1).half()# N, space_dim + 1, L, D 
                # Add the learnable tokens of this layer with the input, replaced by previous
                # layer learnable tokens
                x = torch.cat([prefix, ctx, suffix], dim=2)
    
        if self.text_layer:# concat vector/space prompts within one sequence
            #(n_cls, space_dim + 1, 77, dim) => (N*, 77, dim)
            n_cls, _, _, dim, = x.size()
            len_one_sentence = 1 + self.n_ctx_text + self.max_name_len #BOS + Prompt + {Name_Len+.+EOS}
            #truncate the input/ remove padding
            x_truncate = x[:,:,:len_one_sentence,:] # n_cls, space_dim + 1, len_one_sentence, dim 
            # calculate sentence per sequence
            max_n  = 77 // len_one_sentence
            x_truncate = x_truncate.flatten(0,1) # (space_dim+1)*n_cls, len_one_sentence, dim

            # (space_dim+1)*n_cls, len_one_sentence, dim => N*, 77, dim
            # if the sentence number is not divisible by max_n, pad more sentence
            last_drop = 0
            if (x_truncate.shape[0] % max_n): 
                last_drop = max_n - (x_truncate.shape[0] % max_n) # pad
                if self.pad1 is None:
                    self.pad1 = torch.zeros_like(x_truncate[-last_drop:]).to(dtype=x.dtype, device=x.device)
                pad = self.pad1 
                x_truncate = torch.cat([x_truncate, pad]) # pad 0
            
            # concate sentence in one sequence
            x_truncate = x_truncate.reshape(-1, max_n * len_one_sentence, dim) #N*/max_n L D
            # if sequence length is less than 77, pad to 77
            if (max_n * len_one_sentence)<77:
                pad = x[0:1,0,x_truncate.shape[1]:,:].expand(x_truncate.shape[0],-1,-1)
                x_truncate = torch.cat([x_truncate, pad],dim=1)
            x = x_truncate.permute(1, 0, 2) 

        x = x + self.attention(self.ln_1(x))
        x = x + self.mlp(self.ln_2(x))

        # reshape to the n_cls, space_dim + 1, 77, dim
        if self.text_layer:
            x = x.permute(1, 0, 2) # N*/max_n L D
            x_truncate = x[:, : max_n * len_one_sentence, :] 
            x_truncate = x_truncate.reshape( -1, len_one_sentence, dim) # N*, len_one_sentence, dim
            if last_drop:# drop the sentence padding, i.e. pad1
                x_truncate = x_truncate[:-last_drop]
            x_truncate = x_truncate.reshape( n_cls, (self.space_dim + 1), len_one_sentence, dim)# n_cls, space_dim+1, L, D

            # recover to the n_cls, space_dim + 1, 77, dim
            if self.pad2 is None:
                self.pad2 = torch.zeros([n_cls, (self.space_dim + 1), 77-len_one_sentence,dim]).to(dtype=x.dtype, device=x.device)
            pad = self.pad2
            x = torch.cat([x_truncate, pad],dim=2)
        return x
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        elif current_trainer == 'TCP':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_TCP(width, heads, attn_mask)
                    for _ in range(layers)])

        elif current_trainer == 'SuPr': 
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock_SuPr(width, heads, attn_mask, True, text_layer, i, design_details) if prompts_needed > i
                                             else ResidualAttentionBlock_SuPr(width, heads, attn_mask, False, text_layer, i, design_details)
                                             for i in range(layers)])
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
        ...
        current_trainer = design_details['trainer']
        if design_details["vision_depth"] == 0 or current_trainer == 'CoOp' or current_trainer == 'CoCoOp':
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.transformer = Transformer(
            ...
            attn_mask=self.build_attention_mask() if "SuPr" not in trainer else self.build_attention_mask_SuPr( 1+design_details["language_ctx"]+design_details["max_name_len"]),
    ...
    def build_attention_mask_SuPr(self, max_smallgrid): 
        # Create a mask of size (context_length, context_length)
        # Create a mask attention to the lower triangular part, constrain within the single sentence
        mask = torch.empty(self.context_length, self.context_length)
        mask.fill_(float("-inf"))
        
        # Number of blocks (segments) to create the lower triangular block
        num_blocks = (self.context_length) // max_smallgrid
        
        for n in range(num_blocks):
            # Calculate the start and end indices for the current block
            start_idx = n * max_smallgrid
            end_idx = (n + 1) * max_smallgrid
            
            # Place the block into the larger mask at the appropriate location
            mask[start_idx:end_idx, start_idx:end_idx].triu_(1)
        if (self.context_length) % max_smallgrid != 0: #prevent nan
            mask[num_blocks * max_smallgrid:, num_blocks * max_smallgrid] = \
                torch.zeros_like(mask[num_blocks * max_smallgrid:, num_blocks * max_smallgrid]).float()
        return mask
